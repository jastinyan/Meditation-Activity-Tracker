...
$checkStmt = $conn->prepare("SELECT role, username, email FROM registeredacc WHERE id_no = ?");
...
// Debug log
error_log("Updating superadmin profile for ID: $id_no");
...
if (!empty($minitial) && !isSingleCapitalLetter($minitial)) {
...
if (hasThreeConsecutiveLetters($lname)) {
...
// Allow single-digit numbers, otherwise validate capitalization
