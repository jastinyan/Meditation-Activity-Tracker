...
async function loadProfileData() {
    try {
        // Get user ID from the displayed element or from PHP session
        let userId = displayIdNo.textContent;
        
        // If still loading or empty, try to get from a data attribute or session
        ...
        const response = await fetch(`get_admin_profile.php?user_id=${encodeURIComponent(userId)}`);
        ...
        if (data.success) {
            ...
            accountInfoEl.innerHTML = `
                ...
                <div class="info-item">
                    ...
                    <span class="info-value"><i class="fas fa-shield-alt" style="color: var(--primary-light);"></i> ${escapeHtml(data.role || 'N/A')}</span>
                </div>
                <div class="info-item">
            ...
            // Show error in the display areas
            personalInfoEl.innerHTML = '<div class="error-message"><i class="fas fa-exclamation-circle"></i> Failed to load personal information</div>';
            addressInfoEl.innerHTML = '<div class="error-message"><i class="fas fa-exclamation-circle"></i> Failed to load address information</div>';
            accountInfoEl.innerHTML = '<div class="error-message"><i class="fas fa-exclamation-circle"></i> Failed to load account information</div>';
        }
    } catch (error) {
        ...
        // Show error in the display areas
        personalInfoEl.innerHTML = '<div class="error-message"><i class="fas fa-exclamation-circle"></i> Error loading data</div>';
        addressInfoEl.innerHTML = '<div class="error-message"><i class="fas fa-exclamation-circle"></i> Error loading data</div>';
        accountInfoEl.innerHTML = '<div class="error-message"><i class="fas fa-exclamation-circle"></i> Error loading data</div>';
    }
}
...
async function loadProfileStats() {
    try {
        ...
        if (data.success) {
            totalSessionsEl.textContent = data.totalSessions || 0;
            totalTimeEl.textContent = data.totalTime || 0;
            currentStreakEl.textContent = data.activeToday || 0;
        } else {
